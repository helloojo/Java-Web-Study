...
public class MySqlProjectDao implements ProjectDao {
  ...
  public Project selectOne(int no) throws Exception {
    ...
    try {
      conn = ds.getConnection();
      stmt = conn.createStatement();
      rs = stmt.executeQuery("SELECT PNAME,CONTENT,STA_DATE,END_DATE,STATE,TAGS FROM PROJECTS WHERE PNO=" + no);
      rs.next();

      return new Project().setNo(no).setTitle(rs.getString("PNAME")).setContent(rs.getString("CONTENT"))
          .setStartDate(rs.getDate("STA_DATE")).setEndDate(rs.getDate("END_DATE")).setState(rs.getInt("STATE"))
